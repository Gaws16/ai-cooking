/**
 * Utility functions for working with the OpenAI API to generate recipes
 */
...
export async function generateRecipe({
  ...
  try {
    ...
    if (!apiKey) {
      throw new Error(
        "OpenAI API key is not defined. Please add NEXT_PUBLIC_OPENAI_API_KEY to your environment variables."
      );
    }
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a helpful cooking assistant that generates creative recipes based on user requirements.",
          },
          {
            role: "user",
            content: prompt,
    ...
    if (!response.ok) {
      ...
      if (
        ...
        throw new Error(
          "OpenAI API quota exceeded. Please check your billing details at https://platform.openai.com/account/billing or update your API key."
        );
      }
